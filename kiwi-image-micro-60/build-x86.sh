...
if [ "$(hostname)" == "ip-10-1-0-96" ]; then
	...
	LIFECYCLE=slmicro60-test
	DISTRIBUTION=slmicro60-test
else 
	echo not krones;
	SUSEMANAGER=susemanager.weiss.ddnss.de
	LIFECYCLE=staging-slmicro60-test
	DISTRIBUTION=slmicro60-test
fi
...
--add-repo http://$SUSEMANAGER/ks/dist/child/$LIFECYCLE-sl-micro-6.0-pool-x86_64-clone/$DISTRIBUTION \
--add-repo http://$SUSEMANAGER/ks/dist/child/$LIFECYCLE-sl-micro-extras-6.0-pool-x86_64/$DISTRIBUTION \
--add-repo http://$SUSEMANAGER/ks/dist/child/$LIFECYCLE-suse-manager-tools-for-sl-micro-6.0-x86_64/$DISTRIBUTION,repo-md,suse-manager-tools-for-sl-micro-6.0-x86_64
exit
