...
<image schemaversion="7.5" name="SL-Micro" displayname="SL Micro">
    ...
    <profiles>
        ...
        <profile name="x86-self_install-encrypted" description="Raw disk for x86_64 - uEFI - with encryption" arch="x86_64">
            <requires profile="bootloader"/>
        </profile>
        <profile name="x86-rt-self_install-encrypted" description="Raw disk for x86_64 - uEFI - with encryption" arch="x86_64">
            <requires profile="bootloader"/>
        </profile>
        <profile name="aarch64-self_install" description="Raw disk for aarch64" arch="aarch64">
            <requires profile="bootloader"/>
        </profile>
        <profile name="aarch64-self_install-encrypted" description="Raw disk for aarch64" arch="aarch64">
            <requires profile="bootloader"/>
        </profile>
        ...
        <profile name="aarch64-rt-self_install-encrypted" description="Raw disk for aarch64 with RT kernel and encryption" arch="aarch64">
            <requires profile="bootloader"/>
        </profile>
    ...
    <preferences profiles="aarch64-self_install,aarch64-rt-self_install">
        ...
        <type
            ...
            firmware="uefi"
            efipartsize="128"
	    kernelcmdline="console=tty0 security=selinux selinux=0 net.ifnames=1"
            bootpartition="false"
            bootkernel="custom"
            devicepersistency="by-uuid"
            btrfs_root_is_snapshot="true"
        ...
        <partitions>
            <partition name="combustion" size="2048" mountpoint="/oem" filesystem="xfs"/>
        </partitions>
    </type>
    </preferences>

    <preferences profiles="x86-self_install-encrypted">
        ...
        <type
            ...
            install_continue_on_timeout="false"
            firmware="uefi"
            kernelcmdline="console=ttyS0,115200 console=tty0 security=selinux selinux=1 net.ifnames=1 rd.kiwi.oem.installdevice=/dev/sda"
            bootpartition="false"
            bootkernel="custom"
        ...
        >
            <oemconfig>
		<!--resize will be done after re-encrypt to speed up the process-->
                <oem-resize>false</oem-resize>
		<!--reboot instead of shutdown  - reboot required due to kexec problem with encryption -->
		...
		<oem-skip-verify>true</oem-skip-verify>
            ...
            <bootloader name="grub2" console="gfxterm" use_disk_password="true" timeout="3" timeout_style="menu" />
            <systemdisk>
                <volume name="home" />
                <volume name="root" />
                <!-- on tmpfs jsc#SMO-2                <volume name="tmp"/> -->
    ...
    <preferences profiles="aarch64-self_install-encrypted,aarch64-rt-self_install-encrypted">
        <version>6.1</version>
        <packagemanager>zypper</packagemanager>
        <bootsplash-theme>SLE</bootsplash-theme>
        <bootloader-theme>SLE</bootloader-theme>
   ...
   <repository type="rpm-md" >
    ...
    <packages type="image" profiles="x86-encrypted,x86-rt-encrypted,x86-self_install-encrypted">
    ...
    <packages type="image" profiles="rpi,aarch64-self_install,x86,x86-encrypted,x86-legacy,x86-self_install,x86-vmware,x86-qcow,aarch64-qcow,s390-kvm,s390-dasd,s390-fba,s390-fcp,ppc64le-512ss,ppc64le-4096ss,ppc64le-512ss-self_install,ppc64le-4096ss-self_install,aarch64-self_install-encrypted,x86-self_install-encrypted">
        <package name="kernel-default"/>
        <package name="kernel-firmware-all"/>
    </packages>
    <packages type="image" profiles="x86-rt,x86-rt-self_install,x86-rt-encrypted,aarch64-rt,aarch64-rt-self_install,aarch64-rt-self_install-encrypted">
    ...
    <packages type="image" profiles="x86,x86-encrypted,x86-rt-encrypted,x86-self_install,x86-legacy,x86-vmware,x86-rt,x86-rt-self_install,x86-qcow,aarch64-qcow,rpi,aarch64-self_install,aarch64-rt,aarch64-rt-self_install,ppc64le-512ss,ppc64le-4096ss,ppc64le-512ss-self_install,ppc64le-4096ss-self_install,aarch64-self_install-encrypted,aarch64-rt-self_install-encrypted,x86-self_install-encrypted">
        <package name="dracut-kiwi-oem-repart"/>
        <package name="dracut-kiwi-oem-dump"/>
    </packages>
    <packages type="image" profiles="rpi,aarch64-self_install,aarch64-rt,aarch64-rt-self_install,aarch64-self_install-encrypted,aarch64-rt-self_install-encrypted">
    ...
    <!-- install nvidia packages for jetson -->
    <packages type="image" profiles="aarch64-self_install,aarch64-rt-self_install,aarch64-rt,aarch64-self_install-encrypted,aarch64-rt-self_install-encrypted">
        <package name="nvidia-jetson-kmp-default"/>
        <package name="nvidia-jetpack-all"/>
        <package name="nvidia-container-toolkit"/>
    </packages>

    <!-- install open-iscsi and nfs-client for longhorn -->
    <packages type="image">
        <package name="open-iscsi"/>
        <package name="nfs-client"/>
    </packages>

    <!-- install salt-minion for SUSE manager integrarion -->
    <packages type="image">
        <package name="venv-salt-minion"/>
    </packages>

    <!-- install elemental agent for rancher integrarion -->
    <packages type="image">
        <package name="elemental-register"/>
        <package name="elemental-system-agent"/>
    </packages>

    <!-- install encyption packages for aarch64 test -->
    <packages type="image" profiles="aarch64-self_install-encrypted">
        <!-- full disk encryption stuff -->
        <package name="device-mapper"/>
        <package name="cryptsetup"/>
        <package name="system-user-tss"/>
